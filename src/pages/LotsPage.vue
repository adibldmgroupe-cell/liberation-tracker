        ...
        <tbody>
          <tr v-for="l in pagedLots" :key="l.id" :class="{'row-sel':isSelected(l.id),'row-bloquante':l.dev_has_bloquante}" @click="goToLot(l.id)">
            ...
            <template v-for="ck in visibleCols" :key="'c-'+ck+'-'+l.id">
              ...
              <!-- Date de réception (entrée stock = date d'enregistrement) -->
              <td v-else-if="CC[ck].r==='date_recep'" class="mono dim">{{l.date_fmt||'—'}}</td>
              <!-- Date de libération -->
              <td v-else-if="CC[ck].r==='date_lib'" class="mono dim">{{l.date_lib||'—'}}</td>
              <td v-else class="mono dim">{{l.date_fmt||'—'}}</td>
            </template>
          </tr>
        </tbody>
      </table>
        ...
        <div v-else-if="dataStats" class="dm-result-wrap">
          <div class="m-result">
            ...
            <div class="m-rg" v-if="dataStats.created !== undefined">
              ...
              <div class="m-rc"><div class="m-rv" style="color:var(--th-accent)">{{dataStats.updated}}</div><div class="m-rl">Màj</div></div>
...
.btn-toggle{font-size:11px;padding:4px 10px;border:1px solid #2563eb;border-radius:3px;background:#dbeafe;cursor:pointer;color:#1d4ed8;font-family:inherit}.btn-toggle:hover{background:#ddd6fe}
...
.statut-item input{cursor:pointer;accent-color:#2563eb;flex-shrink:0}
.statut-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0}
.statut-lbl{flex:1}
.btn-cols{font-size:11px;padding:4px 10px;border:1px solid #ddd;border-radius:3px;background:#fff;cursor:pointer;color:#666;font-family:inherit;white-space:nowrap}.btn-cols:hover{background:#f5f5f5}.btn-cols-on{border-color:#2563eb;background:#dbeafe;color:#1d4ed8}
...
.col-item input{cursor:pointer;accent-color:#2563eb}
...
.col-item-over{background:#dbeafe;border-color:#2563eb}
...
.fbtn:hover{border-color:#ccc}.fbtn.active{border-color:#2563eb;background:#dbeafe;color:#1d4ed8}
...
.tb{width:100%;border-collapse:collapse;font-size:11px;white-space:nowrap}.tb th{font-size:9px;text-transform:uppercase;color:#2563eb;font-weight:700;padding:5px 4px;text-align:left;border-bottom:1px solid #dbeafe;position:sticky;top:0;background:#eff6ff;z-index:1}
.sortable{cursor:pointer;user-select:none}.sortable:hover{color:#2563eb}.sort-arrow{font-size:10px;color:#ccc}
...
.s-quarantaine{background:#FAEEDA;color:#854F0B}.s-accepte{background:#EAF3DE;color:#3B6D11}.s-sous_investigation{background:#FCEBEB;color:#A32D2D}.s-vide{background:#f5f5f5;color:#999}.s-enprod{background:#dbeafe;color:#1d4ed8}.s-enprep{background:#F0EBFE;color:#5B3CC4}.s-refuse{background:#e8e8e8;color:#333}
...
.dc-ok{background:#EAF3DE;color:#3B6D11}.dc-ret{background:#FCEBEB;color:#A32D2D}.dc-wait{background:#f5f5f5;color:#999}.dc-prog{background:#dbeafe;color:#1d4ed8}.dc-na{background:transparent;color:#ccc}.dc-date{background:#fafafa;color:#666;font-family:'SF Mono',monospace}
...
.plan-date{font-size:9px;font-family:'SF Mono',monospace;padding:2px 5px;border-radius:2px;background:#eff6ff;color:#2563eb;font-weight:500;cursor:pointer}
.plan-revised{background:#fff3cd;color:#856404}
.plan-empty{font-size:14px;color:#ddd;cursor:pointer;line-height:1}.td-plan:hover .plan-empty{color:#2563eb}
...
.cf-chip{display:flex;align-items:center;gap:4px;background:#dbeafe;color:#1d4ed8;padding:2px 8px;border-radius:10px;font-size:11px}
.cf-chip strong{font-weight:600}
.cf-rm{background:none;border:none;cursor:pointer;color:#2563eb;font-size:11px;padding:0 0 0 2px;line-height:1}
...
.th-txt.sortable{cursor:pointer}.th-txt.sortable:hover{color:#2563eb}
.th-f{background:none;border:none;cursor:pointer;color:#ccc;font-size:11px;padding:0 2px;line-height:1;border-radius:2px;flex-shrink:0;transition:.1s}.th-f:hover{color:#2563eb;background:#f0f0f0}
.th-f-on{color:#2563eb !important;background:#dbeafe}
/* column dropdown */
...
.col-dd-item:hover{background:#f5f5f5}.col-dd-all{color:#999;font-style:italic;border-bottom:1px solid #f0f0f0}.col-dd-on{background:#dbeafe;color:#1d4ed8;font-weight:500}
/* Inline action menu */
...
.inline-act{display:block;width:100%;padding:8px 12px;text-align:left;border:none;background:#fff;cursor:pointer;font-size:12px;font-family:inherit;border-bottom:1px solid #f8f8f8;transition:.1s}.inline-act:hover{background:#dbeafe;color:#1d4ed8}
.inline-empty{padding:10px 12px;font-size:11px;color:#999;text-align:center}
.inline-hist-toggle{display:block;width:100%;padding:6px 12px;text-align:left;border:none;border-top:1px solid #f0f0f0;background:#fafafa;cursor:pointer;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.6px;color:#aaa;font-family:inherit;transition:.1s}.inline-hist-toggle:hover{color:#2563eb;background:#eff6ff}
...
.inline-motif-title{font-size:11px;font-weight:600;color:#1d4ed8;padding:8px 12px 6px;border-bottom:1px solid #f0f0f0}
...
.inline-motif-input:focus{border-color:#2563eb}
.inline-motif-btns{display:flex;gap:6px;padding:0 12px 10px}
.inline-motif-confirm{flex:1;padding:5px 0;font-size:11px;font-weight:600;font-family:inherit;background:#2563eb;color:#fff;border:none;border-radius:3px;cursor:pointer;transition:.15s}.inline-motif-confirm:hover{background:#1d4ed8}
...
.dp-input{width:100%;padding:6px 8px;border:1px solid #ddd;border-radius:3px;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box}.dp-input:focus{border-color:#2563eb}
...
.charge-low{color:#2563eb;font-weight:500}
...
.dp-hist-val{display:block;font-size:12px;font-weight:600;color:#2563eb;font-family:'SF Mono',monospace}
.dp-hist-meta{display:block;font-size:10px;color:#999;margin-top:1px}
.dp-actions{display:flex;gap:6px;margin-top:8px}
.dp-ok{flex:1;padding:6px;background:#2563eb;color:#fff;border:none;border-radius:3px;cursor:pointer;font-size:12px;font-weight:500}.dp-ok:hover{background:#1d4ed8}
...
.dev-pop-acc-hd{display:flex;align-items:center;gap:5px;padding:5px 8px;cursor:pointer;background:#fafafa;user-select:none;transition:.1s}.dev-pop-acc-hd:hover{background:#eff6ff}
...
.dev-pop-svc{background:#dbeafe;color:#1d4ed8;padding:1px 6px;border-radius:10px;font-size:9px;font-weight:600;white-space:nowrap}
...
.dev-pop-in-sm{width:100%;padding:4px 7px;border:1px solid #e0e0e0;border-radius:3px;font-size:12px;font-family:inherit;outline:none;box-sizing:border-box}.dev-pop-in-sm:focus{border-color:#2563eb}
.dev-pop-ta-sm{width:100%;padding:4px 7px;border:1px solid #e0e0e0;border-radius:3px;font-size:12px;font-family:inherit;resize:vertical;outline:none;box-sizing:border-box}.dev-pop-ta-sm:focus{border-color:#2563eb}
.dev-save-btn{font-size:10px;padding:3px 10px;border:1px solid #2563eb;border-radius:3px;background:#dbeafe;color:#1d4ed8;cursor:pointer;margin-top:5px;font-family:inherit}.dev-save-btn:hover{background:#ddd6fe}
...
.action-trigger:hover{border-color:#2563eb;color:#333}
.action-trigger-on{border-color:#2563eb;color:#1d4ed8;background:#dbeafe}
...
.ap-item:hover{background:#dbeafe;color:#1d4ed8}
.ap-item-active{background:#dbeafe;color:#1d4ed8;font-weight:600}
.ap-item-arr{color:#d0d0d0;font-size:14px;flex-shrink:0;line-height:1}
.ap-item:hover .ap-item-arr,.ap-item-active .ap-item-arr{color:#2563eb}
...
.ap-acc-hd:hover{background:#eff6ff;color:#2563eb}
.ap-acc-chevron{font-size:12px;color:#aaa;flex-shrink:0;width:12px}
.ap-acc-label{flex:1}
.ap-acc-cnt{font-size:10px;color:#1d4ed8;background:#dbeafe;border-radius:8px;padding:1px 6px;font-weight:600}
.ap-acc-items{padding:0 0 4px 0;background:#faf5ff}
.bulk-btn{padding:5px 14px;font-size:12px;font-weight:500;background:#2563eb;color:#fff;border:none;border-radius:3px;cursor:pointer;white-space:nowrap}.bulk-btn:hover{background:#1d4ed8}.bulk-btn:disabled{opacity:.35;cursor:not-allowed}
.bulk-info{font-size:11px;color:#2563eb;font-family:'SF Mono',monospace}
...
.bulk-date-in{padding:5px 8px;border:1px solid #ddd;border-radius:3px;font-size:12px;font-family:inherit;outline:none}.bulk-date-in:focus{border-color:#2563eb}
/* checkboxes */
.th-chk,.td-chk{width:32px;text-align:center;padding:0 4px !important}.td-chk{cursor:pointer}.row-sel td{background:#dbeafe !important}
/* modal masse */
...
.m-chip{font-size:11px;font-family:'SF Mono',monospace;padding:2px 8px;background:#f5f5f5;border-radius:2px;color:#666}.m-more{background:#dbeafe;color:#2563eb}
.m-actions{display:flex;gap:8px;margin-top:16px}
.m-btn-ok{flex:1;padding:11px;background:#2563eb;color:#fff;border:none;font-size:13px;font-weight:500;cursor:pointer;border-radius:2px;min-height:44px}.m-btn-ok:hover{background:#1d4ed8}.m-btn-ok:disabled{opacity:.5}
...
.btn-data{font-size:11px;padding:4px 10px;border:1px solid var(--th-accent,#2563eb);border-radius:3px;background:var(--th-bg3,#eff6ff);cursor:pointer;color:var(--th-accent,#1d4ed8);font-family:inherit;font-weight:600;white-space:nowrap}
...
.dm-url:focus{border-color:var(--th-accent,#2563eb)}
.dm-btn-primary{width:100%;padding:10px;font-size:13px;font-weight:600;font-family:inherit;background:#2563eb;color:#fff;border:none;border-radius:5px;cursor:pointer}.dm-btn-primary:hover:not(:disabled){background:#1d4ed8}.dm-btn-primary:disabled{opacity:.45;cursor:not-allowed}
...
.dm-prog-fill{height:100%;background:#2563eb;border-radius:3px;transition:width .3s}
