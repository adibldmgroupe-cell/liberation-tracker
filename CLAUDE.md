...
MàJ IF/IC (Mise à jour de l'instruction de fabrication/conditionnement), MàJ Nmcl OF/OC (Mise à jour de la
nomenclature de fabrication/conditionnement), Clôt. SAP OF/OC (Clôture SAP de l'ordre de fabrication/
conditionnement). ⚠️ **Pas d'AR** pour : les MàJ (`isMajDoc` — flux standard 3 étapes Émission→Vérif AQ→
Approbation DT) ET la clôture SAP (`isClotSap` — flux PROPRE 4 étapes : Demande validation→Validation Planif→
Demande clôture→Clôture, doc créé directement à `emis`). Standard IF/IC/DA/CCL/RVP = 5 clics (avec AR).
